...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    # 这里有点儿需要注意，必须模拟这么多次，因此没法提前return了
    result_list=[]
    while(num_rolls>0):
        result_list.append(dice())
        num_rolls-=1
    return 1 if 1 in result_list else sum(result_list)
...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    "*** YOUR CODE HERE ***"
    # 这块儿不用写一个真的求最大公约数的函数，毕竟你又没学过辗转相除。
    # 从10开始，最大就是100,直接遍历
    i=10
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while(max(score0, score1)<goal):
        if who==0:
            ...
            tmp_score = take_turn(num_rolls, score1, dice)
            score0 += tmp_score
            while(extra_turn(score0,score1)and max(score0, score1)<goal):
            ...
            tmp_score = take_turn(num_rolls, score0, dice)
            score1 += tmp_score
            while(extra_turn(score1,score0) and max(score0, score1)<goal):
                say=say(score0,score1)
                num_rolls = strategy1(score1, score0)
                tmp_score = take_turn(num_rolls, score0, dice)
                score1 += tmp_score
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0,score1):
        if who==0:
            tmp_inc=score0-last_score
            if tmp_inc>running_high:
                print(f"{tmp_inc} point(s)! The most yet for Player {who}")
                return announce_highest(who,score1,tmp_inc)
            else:
                return announce_highest(who,score0,running_high)
        if who==1:
            tmp_inc=score1-last_score
            if tmp_inc>running_high:
                print(f"{tmp_inc} point(s)! The most yet for Player {who}")
                return announce_highest(who,score1,tmp_inc)
            else:
