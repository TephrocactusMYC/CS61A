...
def balanced(m):
    ...
    if is_planet(m):
        ...
        ll,lr=total_weight(end(left(m))),total_weight(end(right(m)))
        wl,wr=length(left(m)),length(right(m))
        if ll*wl==lr*wr and balanced(end(left(m))) and balanced(end(right(m))):
            return True
        return False
...
def preorder(t):
    ...
    tree_list=[]
    def helper(tmp_tree):
        tree_list.append(label(tmp_tree))
        for branch in branches(tmp_tree):
            helper(branch)
        return tree_list
    return helper(t)
...
def has_path(t, word):
    ...
    if len(word)==1:
        ...
        return True in [has_path(branch,word[1:]) for branch in branches(t) if label(t)==word[0]]
...
def multiple_references_explanation():
    return """The same variable may have multiple values."""
