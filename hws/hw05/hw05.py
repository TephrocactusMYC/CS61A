class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self,kind,price,amount=0,stock=0):
        self.kind=kind
        self.price=price
        self.amount=amount
        self.stock=stock
    def vend(self):
        ...
        if self.amount<self.price:
            return 'You must add ${0} more funds.'.format(self.price-self.amount)
            ...
            self.stock-=1
            tmp=self.amount-self.price
            self.amount=0
            return 'Here is your {} and ${} change.'.format(self.kind,tmp)
    def restock(self,number):
        self.stock+=number
        return 'Current {} stock: {}'.format(self.kind,self.stock)
    ...
    def add_funds(self,number):
        if self.stock>0:
            ...
            return 'Inventory empty. Restocking required. Here is your ${}.'.format(number)
...
def store_digits(n):
    ...
    if n<10:
        ...
        while(tmp>=10):
            tmp//=10
            count+=1
        first,all_but_first=tmp,n%(10**count)
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        return is_bst(left_tree) and is_bst(right_tree) and bst_max(left_tree)<=t.label and bst_min(right_tree)>t.label
